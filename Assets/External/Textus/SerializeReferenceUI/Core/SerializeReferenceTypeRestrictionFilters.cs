...
public static class SerializeReferenceTypeRestrictionFilters
{
    ...
    public static Func<Type, bool> TypeIsEqual(Type[] types) => type => types.Any(e => TypeIsEqual(type, e));
    public static Func<Type, bool> TypeIsNotEqual(Type[] types) => type => types.All(e => !TypeIsEqual(type, e));
    public static bool TypeIsSubclassOrEqual(Type type, Type comparator) =>  type.IsSubclassOf(comparator) || type == comparator;
    
    public static bool TypeIsEqual(Type type, Type comparator) =>  type == comparator;
